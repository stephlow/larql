...
pub fn predict_q4k_metal_hidden(
    ...
    let layers: Vec<_> = (0..num_layers).map(|layer| {
        let (wq, wk, wv, wo) = resolve_attn_weights(index, layer)
            .expect("attn Q4K slices missing");
        let [(gate_bytes, gate_fmt), (up_bytes, up_fmt), (down_bytes, down_fmt)] =
            index.interleaved_q4k_layer_data(layer).expect("ffn Q4K slices missing");
        fn to_fmt(s: &str) -> QuantFormat {
            match s {
                "Q4_K" => QuantFormat::Q4_K,
                "Q6_K" => QuantFormat::Q6_K,
        ...
        build_arch_params(
            weights, layer, wq, wk, wv, wo,
            larql_compute::QuantWeight { data: gate_bytes, scales: None, format: to_fmt(gate_fmt) },
            larql_compute::QuantWeight { data: up_bytes, scales: None, format: to_fmt(up_fmt) },
            larql_compute::QuantWeight { data: down_bytes, scales: None, format: to_fmt(down_fmt) },
        )
    }).collect();
    ...
    let mut x_all = Vec::with_capacity(seq_len * hidden);
    for &tok in token_ids {
        x_all.extend(weights.embed.row(tok as usize).iter().map(|v| v * embed_scale));
    }
