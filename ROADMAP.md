...
- **larql-compute coverage audit + improvement** (2026-05-09): `cargo llvm-cov` reports **56.03% → 58.45% line coverage** (+2.42 pp). Biggest single win: deleting `crates/larql-compute/src/metal/prefill.rs` (591 LoC of confirmed-orphan code, all `#[allow(dead_code)]`, no callers — production prefill goes through `prefill_q4` → `dispatch_full_pipeline`). Added 6 targeted tests: 2 for the new `residual_norm_store` D-RMS-FUSE shader (parity with separate ops + offset variant), 2 for `metal/stages/qk_norm.rs::tg_width` (helper coverage 0% → 23%), 2 for `metal/stages/layer_scalar.rs::encode` (12% → 97%). Coverage policy at `crates/larql-compute/coverage-policy.json` sets a 90%/93.5% target — current is well below; the policy is documentation-as-aspiration, not a CI gate today. Largest remaining gaps: `metal/trait_impl/decode.rs` (627 LoC at 21% — heavy MoE / split-profile branches) and `metal/stages/ffn.rs` (227 LoC at 44% — needs synthetic Q4_K weight setup).
